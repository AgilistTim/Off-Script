...
export class ReportDataAggregationService {
  ...
  static async aggregateUserData(
    ...
    try {
      ...
      // Execute all data collection operations in parallel for efficiency
      const [
        profile,
        careerJourney,
  ...
  static async getConversationAnalytics(
    ...
    try {
      ...
      const allTopics = chatSummaries.flatMap(summary => 
        [...(summary.interests || []), ...(summary.careerGoals || []), ...(summary.skills || [])]
      );
  ...
  static async getSkillsData(
    ...
    try {
      ...
      const allSkills = chatSummaries.flatMap(summary => summary.skills || []);
