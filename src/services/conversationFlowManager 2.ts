...
export class ConversationFlowManager {
  ...
  shouldEnableCareerTools(): boolean {
    ...
    // During onboarding, enable tools progressively after collecting basic info
    // Enable after 4+ messages (name + initial questions answered)
    if (phase.phase === 'onboarding' && messageCount >= 4) {
      return true;
    }
